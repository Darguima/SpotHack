...
const DownloadMusicPage: React.FC<StackScreenProps<any>> = ({navigation}) => {
  ...
  const [playlistsOnQueue, setPlaylistsOnQueue] = useState(
    [] as playlistsOnQueueSchema,
  );
  ...
  const onFocus = useCallback(() => {
    ...
    if (queueChangesListenerIdentifier === undefined) {
      const identifier = downloadMachine.addQueueChangesListener(
        refreshDownloadMachineInfo,
      );

      queueChangesListenerIdentifier = identifier;
    } else {
      downloadMachine.changeQueueChangesListener(
  ...
  // Filtering Musics
  const getFilteredData = (data: musicOnQueueSchema[]) =>
    data.filter(music => {
      if (
        (!showAlreadyDownloadedMusics && music.stage === 'alreadyDownloaded') ||
        (playlistIdToFilter !== 'all' &&
          music.playlistId !== playlistIdToFilter)
      ) {
        return false;
      } else {
        return true;
    ...
    <View style={styles.container}>
      ...
      {!errorMessage && (
        <FlatList
          ...
          ListFooterComponent={
            flatListEndReached ||
            downloadStatistics.queueLength -
              downloadStatistics.alreadyDownloadedMusics ===
              0 ? (
          ...
          onScrollBeginDrag={() => {
            setFlatListEndReached(false);
          }}
          onEndReached={() => {
            setFlatListEndReached(true);
          }}
