...
const UpdatePlaylistsButtons:React.FC = () => {
	...
	const addedMusicsNumber = playlistsChanges
		.reduce((number, playlist) => (number + playlist.added.length), 0)

	const removedMusicsNumber = playlistsChanges
		.reduce((number, playlist) => (number + playlist.removed.length), 0)
	...
	const downloadAllPlaylists = async () => {
		...
		const downloadStatus = await downloadMachine.addMusicsToDownloadQueue(musicsToDownload)
		...
		updatePlaylistsChanges(
			playlistsChanges
				...
				.filter(playlistChange => {
					if (playlistChange.added.length === 0 && playlistChange.removed.length === 0) {
						return false
					} else {
						return true
					}
				...
				<View style={styles.contentContainer}>
					...
					<View style={styles.buttonContainer}>
						...
						>
							<Entypo
								name={'download'}
								style={ addedMusicsNumber !== 0 ? styles.buttonIconEnabled : styles.buttonIconDisabled }
								size={20}
							/>

							<View style={styles.textContainer}>

								<Text style={addedMusicsNumber !== 0 ? styles.textEnabled : styles.textDisabled} >
									Download
								</Text>
					...
					<View style={styles.buttonContainer}>
						...
						>
							<Entypo
								name={'trash'}
								style={ removedMusicsNumber !== 0 ? styles.buttonIconEnabled : styles.buttonIconDisabled }
								size={20}
							/>

							<View style={styles.textContainer}>

								<Text style={removedMusicsNumber !== 0 ? styles.textEnabled : styles.textDisabled} >
									Delete
								</Text>

								<Text style={removedMusicsNumber !== 0 ? styles.textEnabled : styles.textDisabled} >
									{removedMusicsNumber} Music{removedMusicsNumber === 1 ? '' : 's'}
								</Text>
