...
const PlaylistsChanges: React.FC = () => {
  ...
  const playlistsNames =
    playlistsChanges.length > 0
      ? playlistsChanges.map(playlist => playlist.playlistName)
      : [];

  const numberOfMusicsToDownload =
    playlistsChanges.length > 0
      ? playlistsChanges
          ...
          <View>
            {numberOfMusicsToDownload !== 0 && (
              <Text style={styles.whiteText}>
                You have{' '}
                <Text style={styles.highlightedText}>
                  {numberOfMusicsToDownload} music
                  {numberOfMusicsToDownload === 1 ? '' : 's'}
                </Text>{' '}
                to download.
              </Text>
            )}
            ...
            {numberOfMusicsToDelete !== 0 && (
              <Text style={styles.whiteText}>
                You have{' '}
                <Text style={styles.highlightedText}>
                  {numberOfMusicsToDelete} music
                  {numberOfMusicsToDelete === 1 ? '' : 's'}
                </Text>{' '}
                to delete.
              </Text>
            )}
