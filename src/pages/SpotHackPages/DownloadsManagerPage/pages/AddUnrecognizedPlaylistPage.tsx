...
import { HomeScreenParamsListProp, homeStackUseNavigator } from '../../../../routes/home.routes'
...
const AddUnrecognizedPlaylistPage:React.FC<Prop> = ({ route: { params: { rootPath, downloadsInfo } } }) => {
	...
	useEffect(() => {
		...
		(async () => {
			const playlistsDetectedNames = Object.keys(downloadsInfo[rootPath])
				.map(playlistId => removeSpecialChars(downloadsInfo[rootPath][playlistId].playlistName))

			const dirItems: DirItemsFiltered[] = (await readDir(rootPath))
				.map(item => ({
					...item,
					ignore: !item.isDirectory() || playlistsDetectedNames.includes(item.name)
				}))

			// To show in order (unrecognized folder, playlists, files)
			const dirs = dirItems.filter(item => item.isDirectory() && !item.ignore)
			const playlists = dirItems.filter(item => item.isDirectory() && item.ignore)
	...
	const sentencesRender = (sentences: string[]) => sentences.map(
		(sentence, index) =>
		...
		<ScrollView style={styles.container}>
			...
			<View style={styles.textContainer}>
				...
				<Text style={styles.descriptionText}>
					If you already downloaded a playlist and for some reason {"it's"} no longer being recognized, you can correct it.
				</Text>
			...
			<View>
				...
				{rootPathItems.map((item, index) => {
					...
					return <MusicPlaylistView
						...
						title={item.name}
						artists={item.ignore ? (isDir ? 'Playlist Already Detected' : 'This is file') : 'Am I a playlist?'}
						imageSource={isDir ? require('../../../../assets/icons/directoryIcon.png') : require('../../../../assets/icons/fileIcon.png')}

						disabled={item.ignore}
						entypoIconName={!item.ignore ? 'chevron-right' : undefined}

						iconPressAction={() => { navigate('ReferencePlaylistPage', { path: item.path, pathName: item.name }) }}
						viewPressAction={() => { navigate('ReferencePlaylistPage', { path: item.path, pathName: item.name }) }}
