...
const PlaylistInfo:React.FC<PlaylistInfoProps> = ({
	...
	const referencePlaylistButtonPress = async () => {
		const newPath = path.substring(0, path.lastIndexOf('/')) + '/' + removeSpecialChars(playlistInfo?.title!)
			...
			<View style={styles.container}>
				<View style={styles.playlistPreview}>
					<View style={styles.playlistCoverContainer}>
						<Image source={playlistInfo.coverImage} style={styles.playlistCover} width={0} height={0} />
					</View>

					<View style={styles.playlistNameOwnerContainer}>
						<Text style={styles.playlistPreviewTitle}>{playlistInfo.title}</Text>
						<Text style={styles.playlistPreviewOwner}>{playlistInfo.owner}</Text>
					</View>
				</View>

				<View style={styles.warmingContainer}>
					<Text style={styles.warmingText}>
						<Text style={styles.warmingTitle}>
							{'Warming: '}
						</Text>
							All files in this folder that are not musics in this playlist can be deleted
					</Text>
