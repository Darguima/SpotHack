...
import downloadManager, { downloadsInfoSchema } from '../../../SpotHack_Core/DownloadManager'

const DownloadsManagerPage:React.FC = () => {
	const { navigate } = homeStackUseNavigator()

	const { playlistsChecked } = useDownloadsInfo()
	const { spotHackSettings: { rootPath: settingsRootPath } } = useSpotHackSettings()
	useEffect(() => {
		setRootPath(settingsRootPath)
	}, [settingsRootPath])

	const [downloadsInfo, setDownloadsInfo] = useState<downloadsInfoSchema>(downloadManager.downloadsInfo)
	...
	return (
		...
		>
			<Header rootPath={rootPath} setRootPath={setRootPath} rootPaths={Object.keys(downloadsInfo)}/>
			...
			<MusicPlaylistView
				...
				viewPressAction={() => navigate('AddUnrecognizedPlaylistPage', { rootPath, downloadsInfo })}
				iconPressAction={() => navigate('AddUnrecognizedPlaylistPage', { rootPath, downloadsInfo })}
			/>
...

			{
				Object.keys(downloadsInfo[rootPath]).map((playlistId, index) => {
					...
					return <MusicPlaylistView
						...
						artists={`${playlist.tracks.length} music${playlist.tracks.length !== 1 ? 's' : ''} downloaded`}
						imageSource={playlist.coverImage}

						style={{
							marginTop: '2%',
							marginBottom: index === Object.keys(downloadsInfo[rootPath]).length - 1 ? '4%' : '2%'
						}}
