...
const RootPathInput: React.FC = () => {
  ...
  const verifyAndSetNewRootPath = async (possibleNewRootPath: string) => {
    const storagePermissions = await getExternalStoragePermissions(() => {
    ...
    if (!possibleNewRootPath.endsWith('/')) {
      possibleNewRootPath += '/';
    }
  ...
  return (
    <ContentBox title={'Root Path'} style={styles.rootPathContainer}>
      <Text style={styles.rootPathDescription}>
        Where your songs are saved:{' '}
      </Text>
      <TextInput
        style={[
          styles.rootPathInput,
          isNewRootPathValid ? {} : styles.invalidRootPath,
        ]}
        value={newRootPath}
        onChangeText={e => {
          setIsNewRootPathValid(true);
      ...
      <View style={styles.buttonsRowContainer}>
        {!rootPathIsEditable ? (
          ...
          <>
            ...
            <View
              style={[
                styles.rootPathButtonContainer,
                isNewRootPathValid ? {} : styles.invalidRootPath,
              ]}>
              <TouchableOpacity
                style={styles.rootPathButtons}
                activeOpacity={0.6}
                onPress={() => {
                  verifyAndSetNewRootPath(newRootPath);
                }}>
                <Text
                  style={[
                    styles.rootPathButtonsText,
                    isNewRootPathValid
                      ? {}
                      : {...styles.invalidRootPath, borderWidth: 0},
                  ]}>
                  Save
                </Text>
              </TouchableOpacity>
