...
const MusicDetailPage: React.FC<MusicDetailPageProps> = ({
  ...
  useEffect(() => {
    let isMounted = true;
    (async () => {
      const response: SpotifyApi.SingleTrackResponse = (
        await spotifyApi.get(`tracks/${spotifyId}`)
      ).data;
      ...
      const newMusicInfo = {
        ...
        image:
          response.album.images.length > 0
        ...
        thumbnail:
          response.album.images.length > 0 ? response.album.images[0].url : '',
        ...
        artistsArray: response.artists.map(item => ({
          name: item.name,
          spotifyId: item.id,
        })),
      };
  ...
  return (
    <View style={styles.container}>
      <ScrollView
        ...
        <ArtistsData artistsArray={musicInfo.artistsArray} />
      </ScrollView>
    </View>
  );
};
