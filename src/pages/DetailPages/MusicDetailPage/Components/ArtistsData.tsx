...
const ArtistsData: React.FC<ArtistsDataProps> = ({artistsArray}) => {
  const [newArtistsArray, setNewArtistsArray] = useState<
    Array<ArtistsDataSchema>
  >([
    {
      spotifyId: '',
  ...
  useEffect(() => {
    try {
      const getArtistsData = Promise.all(
        artistsArray.map(async item => {
          if (item.spotifyId) {
            const spotifyUserData: SpotifyApi.ArtistObjectFull = (
              await spotifyApi.get(`artists/${item.spotifyId}`)
            ).data;
            ...
            return {
              ...
              image:
                spotifyUserData.images.length > 0
                  ? {
                      uri: (
                        spotifyUserData.images[1] || spotifyUserData.images[0]
                      ).url,
                    }
                  : require('../../../../assets/graySquare.jpg'),

              genres: convertArrayToString(
                spotifyUserData.genres.length > 0
