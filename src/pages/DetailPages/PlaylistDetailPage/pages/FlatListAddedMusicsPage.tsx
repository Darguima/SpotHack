...
const FlatListAddedMusicsPage:React.FC<Props> = ({ route: { params: { playlistId } } }) => {
	...
	const downloadAllMusic = async () => {
		const musicsToDownload = addedMusics
			.map(music => ({
				spotifyId: music.spotifyId,
				youtubeId: '',
	...
	const renderItem = ({ item, index }: {item: addedMusicToPlaylistInfoSchema, index: number}) => (
		<MusicPlaylistView
			key={index}
					...
					<>
						<View style={styles.header}>
							...
							<Text style={styles.title}>
								{addedMusics.length} Added Music{addedMusics.length !== 1 ? 's' : ''}
							</Text>
						</View>
							...
							<View style={styles.buttonContainer}>
								...
								>
									...
									<View style={styles.textContainer}>
										...
										<Text style={styles.text} >
											{addedMusics.length} Music{addedMusics.length === 1 ? '' : 's'}
										</Text>
