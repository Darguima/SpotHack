...
const FlatListRemovedMusicsPage: React.FC<Props> = ({
  route: {
    params: {playlistId},
  },
}) => {
  ...
  const removedMusics = (playlistsChangesObject[playlistId] || {removed: []})
    .removed;
  ...
  const deleteMusic = async (musicPath: string, index: number) => {
    ...
    updatePlaylistsChanges(
      Object.keys(playlistsChangesObject).map(
        playlistId => playlistsChangesObject[playlistId],
      ),
    );
  };
  ...
  const deleteAllMusics = async () => {
    await Promise.all(
      removedMusics.map(async music => await deleteFile(music.path)),
    );

    playlistsChangesObject[playlistId].removed = [];

    if (
      playlistsChangesObject[playlistId].added.length === 0 &&
      playlistsChangesObject[playlistId].removed.length === 0
    ) {
  ...
  const renderItem = ({
    ...
    item: removedMusicToPlaylistInfoSchema;
    index: number;
  }) => (
    <MusicPlaylistView
      key={index}
          ...
          <>
            <View style={styles.header}>
              ...
              <Text style={styles.title}>
                {removedMusics.length} Removed Music
                {removedMusics.length !== 1 ? 's' : ''}
              </Text>
            </View>
            ...
            {removedMusics.length > 0 && (
              <View style={styles.buttonContainer}>
                <TouchableOpacity
                  ...
                  <Entypo name={'trash'} style={styles.buttonIcon} size={20} />

                  <View style={styles.textContainer}>
                    <Text style={styles.text}>Delete</Text>

                    <Text style={styles.text}>
                      {removedMusics.length} Music
                      {removedMusics.length === 1 ? '' : 's'}
                    </Text>
                  </View>
                </TouchableOpacity>
