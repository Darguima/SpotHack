...
interface FlatListSpotifyApiTrackResponseFiltered
...
const FlatListMusics: React.FC<StackScreenProps<any>> = ({route: {params}}) => {
  const musicsArray: Array<SpotifyApi.PlaylistTrackObject> =
    params!.musicsArray || [];

  const {navigate, goBack} = useNavigation();

  const tracksArray: Array<FlatListSpotifyApiTrackResponseFiltered> =
    musicsArray.map((item, index) => {
  ...
  const renderItem = ({
    ...
    item: FlatListSpotifyApiTrackResponseFiltered;
  }) => (
    <MusicPlaylistView
      key={item.index}
      style={{
