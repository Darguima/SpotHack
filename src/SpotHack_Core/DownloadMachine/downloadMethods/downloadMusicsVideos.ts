...
export default async function downloadMusicsVideos(this: DownloadMachine) {
  if (this.isDownloadMusicsVideosActive === true) {
    return 0;
  }
  ...
  while (this.downloadMusicsVideosQueue.length > 0) {
    ...
    try {
      ...
      if (assetsCreation === 0) {
        throw new Error('error creating asset on temp path');
      }

      const downloadSuccess = await downloadVideo(
        downloadUrl,
        temporaryPathWithFile,
        queue,
        queueIndex,
      );
      if (downloadSuccess.statusCode !== 200) {
        throw new Error(
          `error on downloading video - ${downloadSuccess.statusCode}`,
        );
      }
      ...
      if (this.isConvertVideosToMusicsActive === false) {
        this.convertVideosToMusics();
      }
    } catch (err) {
      // downloadsStatistics
