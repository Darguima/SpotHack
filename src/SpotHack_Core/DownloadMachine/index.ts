...
type queueChangesListenerFunction = (
  index: string,
  newMusicInfo: musicOnQueueSchema,
  prevMusicInfo: unknown,
  name: string,
) => void;
...
export class DownloadMachine {
  protected queue = onChange(
    ...
    (path: string, value: unknown, previousValue: unknown, name: string) => {
      this.queueChangesListenerFunctions.forEach(eventFunction =>
        eventFunction(
          path,
          value as musicOnQueueSchema,
          previousValue as musicOnQueueSchema,
          name,
        ),
      );
    },
  ...
  private queueChangesListenerFunctions: queueChangesListenerFunctionsSchema =
    [];
  public addQueueChangesListener = (
    eventFunction: queueChangesListenerFunction,
  ) => {
    this.queueChangesListenerFunctions.push(eventFunction);
    return this.queueChangesListenerFunctions.length - 1;
  };

  public changeQueueChangesListener = (
