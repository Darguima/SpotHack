...
interface addMusicsToDownloadQueueReturnSchema {
  ...
  msg:
    ...
    | 'Updating playlists - wait a moment'
    | 'Starting update the playlists - wait a moment';
}

export default async function addMusicsToDownloadQueue(
  this: DownloadMachine,
  playlist: Array<musicForQueueSchema>,
): Promise<addMusicsToDownloadQueueReturnSchema> {
  if (!this.storagePermissions) {
    this.storagePermissions = await getExternalStoragePermissions();

    if (!this.storagePermissions) {
      return {successCode: 0, msg: 'Storage Permissions missing'};
    }
  }
  ...
  playlist.forEach(item => {
    ...
    const alreadyOnQueueMusic =
      this.queue[
        this.queueIds.indexOf(createQueueId(item.spotifyId, item.playlistId))
      ];
    ...
    if (alreadyOnQueueMusic && alreadyOnQueueMusic.progress !== 0) {
      ...
      this.downloadsStatistics.errors.splice(
        this.downloadsStatistics.errors.findIndex(
          ytQuery => ytQuery === alreadyOnQueueMusic.youtubeQuery,
        ),
        1,
      );
    ...
    if (playlistsOnRootPathInfo[item.playlistId]) {
      if (
        playlistsOnRootPathInfo[item.playlistId].tracks.some(
          track => track.spotifyId === item.spotifyId,
        )
      ) {
    ...
    if (
      !this.playlistsOnQueue.some(
        playlist => musicInfo.playlistId === playlist.playlistId,
      )
    ) {
  ...
  if (this.isGetYoutubeIdsActive === false) {
    this.getYoutubeIds();
  }
