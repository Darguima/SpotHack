	...
	public async addDownloadedMusicInfo (
		...
		if (this.playlistsChanges[this.rootPath][playlistId]) {
			...
			if (indexFromPlaylistsChanges !== -1) {
				this.playlistsChanges[this.rootPath][playlistId].added.splice(indexFromPlaylistsChanges, 1)

				if (this.playlistsChanges[this.rootPath][playlistId].added.length === 0 && this.playlistsChanges[this.rootPath][playlistId].removed.length === 0) {
	...
	protected async getApiUpdatedPlaylist (playlistId: string): Promise<downloadedPlaylistInfoSchema | undefined> {
		if (!this.apiUpdatedPlaylists[playlistId]) {
			...
			this.apiUpdatedPlaylists[playlistId] = {
				...
				coverImage: apiPlaylist.images.length > 0
