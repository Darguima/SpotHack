...
export default async (
	...
	for (const pathIndex in playlistsPaths) {
		...
		await Promise.all(
			playlistsOnPath
				.map(async playlist => {
					const filesNames = (await RNFS.readDir(path + removeSpecialChars(playlist.playlistName)))
					...
					const tracks = apiUpdatedPlaylists[playlist.playlistId].tracks
						.filter(trackOnApiPlaylist => filesNames.includes(removeSpecialChars(trackOnApiPlaylist.youtubeQuery) + '.mp3'))
