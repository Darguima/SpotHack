...
import downloadManager, {
  playlistChangesSchema,
  playlistsChangesOnPathSchema,
...
interface DownloadsInfoContextData {
  ...
  updatePlaylistsChanges: (
    newPlaylistsChanges: playlistChangesSchema[],
  ) => void;
...
const DownloadsInfoContext = createContext<DownloadsInfoContextData>(
  {} as DownloadsInfoContextData,
);

export const DownloadsInfoProvider: React.FC = ({children}) => {
  const {
    spotHackSettings: {rootPath},
  } = useSpotHackSettings();

  const [playlistsChanges, setPlaylistsChanges] = useState<
    Array<playlistChangesSchema>
  >([]);
  const [playlistsChangesObject, setPlaylistsChangesObject] =
    useState<playlistsChangesOnPathSchema>({});

  const [playlistsChecked, setPlaylistsChecked] = useState<boolean>(false);

  const updatePlaylistsChanges = (
    newPlaylistsChanges: playlistChangesSchema[],
  ) => {
    const newPlaylistsChangesOnRootPath: playlistsChangesOnPathSchema =
      newPlaylistsChanges.reduce(
        (finalObj, playlist) => ({
          ...finalObj,
  ...
  useEffect(() => {
    let isMounted = true;
    downloadManager.addOnPlaylistUpdateEventFunction(
      (_, newPlaylistsChanges, __, arePlaylistsUpdated) => {
        if (!isMounted) {
          return;
        }
        setPlaylistsChangesObject(newPlaylistsChanges);
        setPlaylistsChanges(
          Object.keys(newPlaylistsChanges).map(
            playlistId => newPlaylistsChanges[playlistId],
          ),
        );
