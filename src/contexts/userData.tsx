...
export const UserDataProvider: React.FC = ({children}) => {
  ...
  const requestSpotifyUserData = useCallback(() => {
    (async () => {
      try {
        const spotifyResponse: SpotifyApi.UserObjectPublic = (
        ...
        const spotifyResponseFiltered = {
          ...
          image:
            (spotifyResponse.images || []).length > 0
              ? {
                  uri: (
                    spotifyResponse.images![1] || spotifyResponse.images![0]
                  ).url,
                }
  ...
  const [userPlaylists, setUserPlaylists] = useState(
    [] as Array<userPlaylistDataSchema>,
  );

  const requestSpotifyUserPlaylistsData = useCallback(() => {
    (async () => {
      try {
        const spotifyResponse: SpotifyApi.ListOfCurrentUsersPlaylistsResponse =
          (
            await spotifyApi.get('me/playlists', {
              params: {
                limit: 50,
              },
            })
