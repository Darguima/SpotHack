...
export const AuthProvider: React.FC = ({children}) => {
  ...
  const [refreshToken, seRefreshToken] = useState<string | undefined>(
    undefined,
  );

  const {spotifyClientId, spotifyBase64Key, isSpotifyLoginPossible} =
  ...
  useEffect(() => {
    const refreshAccessToken = async () => {
      if (refreshToken) {
        const newAccessToken = await spotifyOAuth.refreshToken(
          refreshToken,
          spotifyClientId,
          spotifyBase64Key,
        );
    ...
    const refreshTokenInterceptor = spotifyApi.interceptors.response.use(
      response => {
        return response;
      },
      async error => {
    ...
    return () => {
      spotifyApi.interceptors.response.eject(refreshTokenInterceptor);
    };
  }, [refreshToken]);
  ...
  useEffect(() => {
    (async () => {
      if (oAuthCode) {
        if (oAuthCode !== 'error') {
          const credentials = await spotifyOAuth.getOauthCredentials(
            oAuthCode,
            spotifyBase64Key,
          );
          ...
          if (credentials.access_token && credentials.refresh_token) {
            if (
              credentials.access_token !== 'error' &&
              credentials.refresh_token !== 'error'
            ) {
  ...
  return (
    <AuthContext.Provider
      value={{loading, errorOnLogin, signed: !!accessToken, logIn, logOut}}>
      {children}
    </AuthContext.Provider>
  );
};
