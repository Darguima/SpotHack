...
interface UserHistoryContextData {
  ...
  addMusicToMusicSearchHistory: (
    musicData: musicsSpotifyResponseSchema,
  ) => void;
  ...
  addPlaylistToPlaylistSearchHistory: (
    playlistData: playlistsSpotifyResponseSchema,
  ) => void;
  removePlaylistFromPlaylistSearchHistory: (spotifyId: string) => void;
}

const UserHistoryContext = createContext<UserHistoryContextData>(
  {} as UserHistoryContextData,
);

export const UserHistoryProvider: React.FC = ({children}) => {
  // Music History

  const [musicSearchHistory, setMusicSearchHistory] = useState<
    Array<musicsSpotifyResponseSchema>
  >([]);

  useEffect(() => {
    (async () => {
      const [[, musicSearchHistoryStored]] = await AsyncStorage.multiGet([
        'musicSearchHistory',
      ]);
  ...
  const addMusicToMusicSearchHistory = (
    musicData: musicsSpotifyResponseSchema,
  ) => {
    ...
    AsyncStorage.setItem(
      'musicSearchHistory',
      JSON.stringify([musicData, ...newMusicSearchHistory]),
    );
  };
  ...
  const removeMusicFromMusicSearchHistory = (spotifyId: string) => {
    ...
    AsyncStorage.setItem(
      'musicSearchHistory',
      JSON.stringify(newMusicSearchHistory),
    );
  };

  // Playlist History

  const [playlistSearchHistory, setPlaylistSearchHistory] = useState<
    Array<playlistsSpotifyResponseSchema>
  >([]);

  useEffect(() => {
    (async () => {
      const [[, playlistSearchHistoryStored]] = await AsyncStorage.multiGet([
  ...
  const addPlaylistToPlaylistSearchHistory = (
    playlistData: playlistsSpotifyResponseSchema,
  ) => {
    ...
    AsyncStorage.setItem(
      'playlistSearchHistory',
      JSON.stringify([playlistData, ...newPlaylistSearchHistory]),
    );
  };
  ...
  const removePlaylistFromPlaylistSearchHistory = (spotifyId: string) => {
    ...
    AsyncStorage.setItem(
      'playlistSearchHistory',
      JSON.stringify(newPlaylistSearchHistory),
    );
  };
