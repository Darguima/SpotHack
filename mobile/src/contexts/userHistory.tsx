...
export const UserHistoryProvider: React.FC = ({ children }) => {
  ...
  useEffect(() => {
    (async () => {
      const [[, playlistSearchHistoryStoraged]] = await AsyncStorage.multiGet(['playlistSearchHistory'])

      if (playlistSearchHistoryStoraged) {
        setPlaylistSearchHistory(JSON.parse(playlistSearchHistoryStoraged))
      }
    })()
  }, [])
