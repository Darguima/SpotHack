...
export const ApiCredentialsProvider: React.FC = ({ children }) => {
	...
	const [credentialsStored, setCredentialsStored] = useState<credentialsStored>({ spotifyClientId: '', spotifyClientSecret: '', youtubeApiKey: '' })
	...
	useEffect(() => {
		(async () => {
			...
			try {
				const encryptedCredentialsStored = await EncryptedStorage.getItem('api_credentials')

				if (encryptedCredentialsStored) {
					const credentials: credentialsStored = JSON.parse(encryptedCredentialsStored)
					...
					newSpotifyClientId = credentials.spotifyClientId || newSpotifyClientId
					newSpotifyClientSecret = credentials.spotifyClientSecret || newSpotifyClientSecret
	...
	const storeSpotifyClientId = async (newClientId: string) => {
		...
		try {
			await EncryptedStorage.setItem('api_credentials', JSON.stringify({ ...credentialsStored, spotifyClientId: newClientId }))
			setCredentialsStored({ ...credentialsStored, spotifyClientId: newClientId })
		} catch {}
	}
	...
	const storeSpotifyClientSecret = async (newClientSecret: string) => {
		...
		try {
			await EncryptedStorage.setItem('api_credentials', JSON.stringify({ ...credentialsStored, spotifyClientSecret: newClientSecret }))
			setCredentialsStored({ ...credentialsStored, spotifyClientSecret: newClientSecret })
		} catch {}
	}
	...
	const storeYoutubeApiKey = async (newApiKey: string) => {
		...
		try {
			await EncryptedStorage.setItem('api_credentials', JSON.stringify({ ...credentialsStored, youtubeApiKey: newApiKey }))
	...
	return (
		<ApiCredentialsContext.Provider value={{
			spotifyClientId,
			spotifyClientSecret,
			spotifyBase64Key,
