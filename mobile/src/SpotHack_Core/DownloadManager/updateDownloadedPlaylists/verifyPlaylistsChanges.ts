...
import { downloadedPlaylistsInfoSchema, downloadsInfoSchema, playlistsChangesSchema, removedMusicToPlaylistInfoSchema } from '../index'
	...
	for (const rootPath in downloadsInfo) {
		...
		for (const playlistId in downloadsInfo[rootPath]) {
			...
			for (const musicIndex in apiUpdatedPlaylists[playlistId].tracks) {
				...
				if (!downloadedPlaylist.tracks.some(({ spotifyId }) => spotifyId === musicInfo.spotifyId)) {
			...
			const filesToRemove = (await RNFS.readDir(rootPath + removeSpecialChars(downloadedPlaylist.playlistName)))
				.map(filesInfo => filesInfo.name)
				.filter(fileName => {
					if (apiUpdatedPlaylists[playlistId].tracks.some(apiMusic =>
				...
				.map(fileName => ({
					path: rootPath + removeSpecialChars(downloadedPlaylist.playlistName) + '/' + fileName,
					playlistName: downloadedPlaylist.playlistName,
			...
			for (const filePathIndex in filesToRemove) {
				...
				const metadata = (await RNFFprobe.getMediaInformation(file.path))
					.getAllProperties() as unknown as {format?: {tags?: {[key: string]: string}}}
			...
			if (playlistsChanges[rootPath][playlistId].added.length === 0 && playlistsChanges[rootPath][playlistId].removed.length === 0) {
